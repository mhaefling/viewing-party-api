  ...
  def self.format_user_details(user)
    { data: 
    {
      ...
      attributes: {
        ...
        if user.viewing_parties.joins(:user_viewings).where(user_viewings: { host: true }).distinct.empty?
          []
        else
        ...
        if user.viewing_parties.joins(:user_viewings).where(user_viewings: { host: false }).distinct.empty?
          []
        else
